...
public class Search {
    ...
    static int len;                             // Length of the actual text
    ...
    public static void main(String[] argv) {
        try {
            ...
            int partLength = len / ntasks;
            int overlap = pattern.length - 1;

            for (int i = 0; i < ntasks; i++) {
                int from = i * partLength;
                int to = from + partLength + overlap; // Adds the overlap to the end of the part
                if (i == ntasks - 1) {
                    // Adjust the last segment to cover the remaining text
                    to = len;
                }
                SearchTask task = new SearchTask(text, pattern, from, to);
                taskList.add(task);
            }
            ...
            for (int run = 0; run < runs; run++) {
                ...
                // The overall result is an ordered list of unique occurrence positions
                result = new LinkedList<Integer>();

                // TODO: Combine future results into an overall result
                for (Future<List<Integer>> future : futures) {
                    // Gets a result of the current task
                    List<Integer> taskResult = future.get();

                    // Combine the task result into the result
